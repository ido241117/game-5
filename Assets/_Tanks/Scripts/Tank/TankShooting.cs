...
namespace Tanks.Complete
{
    public class TankShooting : MonoBehaviour
    {
        ...
        public float m_MinLaunchForce = 8f;        // The force given to the shell if the fire button is not held. (Increased from 5)
        [Tooltip("The speed in unit/second the shell have when fired at max charge")]
        public float m_MaxLaunchForce = 30f;        // The force given to the shell if the fire button is held for the max charge time. (Increased from 20)
        [Tooltip("The maximum time spent charging. When charging reach that time, the shell is fired at MaxLaunchForce")]
        public float m_MaxChargeTime = 0.5f;       // How long the shell can charge for before it is fired at max force. (Reduced from 0.75)
        [Tooltip("The time that must pass before being able to shoot again after a shot")]
        public float m_ShotCooldown = 0.5f;         // The time required between 2 shots (Reduced from 1.0)
        [Header("Shell Properties")]
        [Tooltip("The amount of health removed to a tank if they are exactly on the landing spot of a shell")]
        public float m_MaxDamage = 150f;                    // The amount of damage done if the explosion is centred on a tank. (Increased from 100)
        ...
        private TankCheat m_TankCheat;              // Reference to cheat system
        ...
        private void Awake()
        {
            ...
            // Get reference to cheat system (if exists)
            m_TankCheat = GetComponent<TankCheat>();
        }
        ...
        private void Fire ()
        {
            ...
            // CHEAT: Apply damage multiplier from cheat system
            if (m_TankCheat != null)
            {
                explosionData.m_DamageMultiplier = m_TankCheat.GetDamageMultiplier();
            }
            else
            {
                explosionData.m_DamageMultiplier = 1f;
            }
